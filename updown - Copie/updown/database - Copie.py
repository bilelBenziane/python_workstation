...
class Engine(object):
    ...
    def clear(self):
        ...
        with con:
            ...
            cur.execute("DELETE FROM euro")
            cur.execute("DELETE FROM dollar")
            cur.execute("DELETE FROM btc")
            cur.execute("DELETE FROM eth")
            cur.execute("DELETE FROM gbp")
    ...
    def create_currencies_table(self):
        ...
        '''
        keys_on = 'PRAGMA foreign_keys = ON'
        stmnt = 'CREATE TABLE currencies(currency_id INTEGER PRIMARY KEY, \
                    currency_name TEXT, \
                    UNIQUE(currency_id, currency_name) \
                    )'
        con = sqlite3.connect(self.db_path)
        with con:
    ...
    def create_euro_table(self):
        ...
        '''
        keys_on = 'PRAGMA foreign_keys = ON'
        stmnt = 'CREATE TABLE euro(currency_id INTEGER PRIMARY KEY, \
                                    currency_name TEXT,\
                                    day-7 REAL, day-6 REAL,day-5 REAL,day-4 REAL,\
                                    day-3 REAL, day-2 REAL,day-1 REAL,day-0 REAL,\
                                    UNIQUE(currency_id, currency_name),\
                                    FOREIGN KEY(currency_id, currency_name) REFERENCES currencies(currency_id, currency_name) ON DELETE SET NULL)'
    ...
    def create_dollar_table(self):
        ...
        stmnt = 'CREATE TABLE dollar(currency_id INTEGER PRIMARY KEY, \
                                    currency_name TEXT,\
                                    day-7 REAL, day-6 REAL,day-5 REAL,day-4 REAL,\
                                    day-3 REAL, day-2 REAL,day-1 REAL,day-0 REAL,\
                                    UNIQUE(currency_id, currency_name),\
                                    FOREIGN KEY(currency_id, currency_name)\
                                    REFERENCES currencies(currency_id, currency_name) ON DELETE CASCADE)'
        #Connects to the database. Gets a connection object
        con = sqlite3.connect(self.db_path)
        with con:
            #Get the cursor object.
    ...
    def create_btc_table(self):
        ...
        '''
        keys_on = 'PRAGMA foreign_keys = ON'
        stmnt = 'CREATE TABLE btc(currency_id INTEGER PRIMARY KEY, \
                                    currency_name TEXT,\
                                    day-7 REAL, day-6 REAL,day-5 REAL,day-4 REAL,\
                                    day-3 REAL, day-2 REAL,day-1 REAL,day-0 REAL,\
                                    UNIQUE(currency_id, currency_name),\
                                    FOREIGN KEY(currency_id, currency_name)\
                                    REFERENCES currencies(currency_id, currency_name) ON DELETE SET NULL)'
        #Connects to the database. Gets a connection object
        con = sqlite3.connect(self.db_path)
        with con:
            #Get the cursor object.
    ...
    def create_eth_table(self):
        ...
        '''
        keys_on = 'PRAGMA foreign_keys = ON'
        stmnt = 'CREATE TABLE eth(currency_id INTEGER PRIMARY KEY, \
                                    currency_name TEXT,\
                                    day-7 REAL, day-6 REAL,day-5 REAL,day-4 REAL,\
                                    day-3 REAL, day-2 REAL,day-1 REAL,day-0 REAL,\
                                    UNIQUE(currency_id, currency_name),\
    ...
    def create_gbp_table(self):
        ...
        '''
        keys_on = 'PRAGMA foreign_keys = ON'
        stmnt = 'CREATE TABLE gbp(currency_id INTEGER PRIMARY KEY, \
                                    currency_name TEXT,\
                                    day-6 REAL,day-5 REAL,day-4 REAL,\
                                    day-3 REAL, day-2 REAL,day-1 REAL,day-0 REAL,\
                                    UNIQUE(currency_id, currency_name),\
                                    FOREIGN KEY(currency_id, currency_name)\
                                    REFERENCES currencies(currency_id, currency_name) ON DELETE SET NULL)'
    ...
    def create_users_table(self):
        ...
        stmnt = 'CREATE TABLE users(user_id INTEGER PRIMARY KEY, \
                                    username TEXT,\
                                    euro INTEGER, dollar INTEGER,btc INTEGER,eth INTEGER,gbp INTEGER\
...
class Connection(object):
    ...
    def _create_user_object(self, row):
        ...
        return {    'registrationdate': reg_date,
    ...
    def modify_user(self, username, user):
        ...
        query2 = 'UPDATE users SET username = ?,euro = ?, \
                                           dollar = ?,btc = ?, \
                                           eth = ?,gbp = ?, \
                                           WHERE user_id = ?'
        #temporal variables
        user_id = None
        p = user
        _euro = p.get('euro', None)
        _dollar = p.get('dollar', None)
        _btc = p.get('btc', None)
        _eth = p.get('eth', None)
        _gbp = p.get('gbp', None)
        #Activate foreign key support
        ...
        if row is None:
            ...
            pvalue = (_euro, _dollar, _btc, _eth, _gbp, user_id)
            cur.execute(query2, pvalue)
            self.con.commit()
    ...
    def append_user(self, username, user):
        ...
        query2 = 'INSERT INTO users(username,euro,dollar,btc,eth,gbp)\
                  VALUES(?,?,?,?,?,?)'
        ...
        if row is None:
            ...
            # Execute the statement
            pvalue = (username, _euro, _dollar, _btc, _eth, _gbp)
            cur.execute(query2, pvalue)
