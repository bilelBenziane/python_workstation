...
class UpdownObject(MasonObject):    
    ...
    def add_control_add_choice(self, username):
        """
        This adds the add-choice control to an object. Intended for the  
        document object. Here you can see that adding the control is a bunch of 
        lines where all we're basically doing is nested dictionaries to 
        achieve the correctly formed JSON document representation. 
        """

        self["@controls"]["updown:add-choice"] = {
            "href": api.url_for(Choices, username=username),
            "title": "Create choice",
            "encoding": "json",
            "method": "POST",
    ...
    def _choice_schema(self):
        schema = {
            "type": "object",
            "properties": {},
            "required": ["user_id", "from_currency","to_currency","date_from","date_to"]
        }
...
class Choices(Resource):
    ...
    def get(self, username):
        ...
        * Media type: Mason
        ...
        envelope.add_control("self", href=api.url_for(Choices, username=username))
        envelope.add_control_add_choice(username)		
        ...
        for choice in user_choices_db:             
            ...
            item.add_control("profile", href=UPDOWN_CHOICE_PROFILE)
            item.add_control("self", href=api.url_for(Choice, choiceid=choice["choice_id"]))
